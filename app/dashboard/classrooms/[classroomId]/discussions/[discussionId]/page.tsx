...
export default function StudentDiscussionDetailPage({ params }: { params: Promise<{ classroomId: string; discussionId: string }> }) {
  const { classroomId, discussionId } = use(params)
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: reps } = await supabase
        .from("discussion_replies").select("*")
  ...
  return (
    <div className="p-6 md:p-8 lg:p-12 max-w-3xl mx-auto">
      <ScrollReveal direction="up">
        <Link href={`/dashboard/classrooms/${classroomId}/discussions`} className="inline-flex items-center gap-1 text-foreground/50 hover:text-foreground mb-6 text-sm transition-colors">
