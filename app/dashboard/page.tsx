...
export default async function Dashboard() {
  ...
  const initialWidgets = [
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {stats.map((stat, i) => (
            <ScrollReveal key={stat.label} direction="up" delay={i * 0.1}>
              <div className={`p-4 h-full rounded-2xl glass-surface border ${stat.border} hover:border-opacity-50 transition-all group`}>
    ...
    {
      id: "achievements",
      content: recentAchievements && recentAchievements.length > 0 ? (
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)]">
      ...
      <div className="relative z-10 space-y-8">
        ...
        <DashboardGrid userId={user.id} initialWidgets={initialWidgets} />
      </div>
    </div>
  )
}
